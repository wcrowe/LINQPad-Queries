...
//   - Insert proc returns ALL PK columns (works for single/composite PK)
//   - No assumptions that PK is a single column (except optional @NewId for single identity PK)
// - Rowversion/timestamp handled correctly as VARBINARY(8) for params/table vars
// ─────────────────────────────────────────────
void Main()
{
...
{
	// DECLARE @InsertedPk TABLE ([K1] type, [K2] type, ...)
	var cols = string.Join(", ", pk.Select(p => $"[{p.Name}] {SqlType(p)}"));
	return $"DECLARE @InsertedPk TABLE ({cols});";
}

static string BuildPkOutputInsertedInto(List<Column> pk)
{
	// OUTPUT inserted.[K1], inserted.[K2] INTO @InsertedPk([K1],[K2])
	var insertedCols = string.Join(", ", pk.Select(p => $"inserted.[{p.Name}]"));
	var intoCols = string.Join(", ", pk.Select(p => $"[{p.Name}]"));
	return $"OUTPUT {insertedCols} INTO @InsertedPk({intoCols})";
}
...
// - Returns a resultset of ALL PK columns after insert.
// - Keeps @NewId OUTPUT only for single identity PK (back-compat with your prior behavior).
// ─────────────────────────────────────────────
static string InsertProc(string schema, string baseName, string table, List<Column> cols, List<Column> pk, Column? identity, Column? created, Column? updated, Column? softDelete, Column? version)
{
	...
	// Capture inserted PKs for BOTH single/composite PK.
	sb.AppendLine("  " + BuildPkTableVarDecl(pk));
	sb.AppendLine($"  INSERT INTO {table} ({string.Join(", ", fields)})");
	sb.AppendLine($"  {BuildPkOutputInsertedInto(pk)}");
	...
	{
		// Return ALL PK values (composite-safe).
...
// ─────────────────────────────────────────────
// Upsert (composite PK supported)
// - Still uses PK parameters for WHERE (works for composite PK).
// - For single identity PK, keeps OUTPUT param behavior.
// ─────────────────────────────────────────────
	...
	Column? softDeleteCol, bool enableSoftDelete)
{
	...
	var pkWhereNoAlias = BuildPkWhere(pk, "t");
	sb.AppendLine($" SELECT {BuildColumnList(columns, pkNames, softDeleteCol, enableSoftDelete)} FROM {table} t WHERE {pkWhereNoAlias};");
...
// ─────────────────────────────────────────────
// Delete / Restore (composite PK supported)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Read procedures (composite PK supported)
// ─────────────────────────────────────────────
