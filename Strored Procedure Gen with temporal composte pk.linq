...
// - Composite primary key support (FULL)
//   - Includes composite keys with an IDENTITY column as part of the PK
// ─────────────────────────────────────────────
void Main()
{
	...
	master.AppendLine("-- Composite PK support (including IDENTITY-in-composite)");
...
// ─────────────────────────────────────────────
// Helper: Build explicit column list for SELECT (ONLY ONCE)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Insert (composite PK safe; supports IDENTITY-in-composite PK via output)
// ─────────────────────────────────────────────
static string InsertProc(string schema, string baseName, string table, List<Column> cols, List<Column> pk, Column? identity, Column? created, Column? updated, Column? softDelete, Column? version)
{
	...
	// FULL composite support: if the PK includes an IDENTITY column (even in a composite PK),
	// expose it as OUTPUT so callers can receive the generated value.
	var identityInPk = identity != null && pk.Any(p => p.Name.Equals(identity.Name, StringComparison.OrdinalIgnoreCase));
	if (identityInPk)
	{
		// OUTPUT-only param (no "= NULL" needed)
		parms.Add($"@{identity!.Name} {SqlType(identity)} OUTPUT");
	}
	...
	if (identityInPk)
	{
		...
		sb.AppendLine($" SELECT @{identity.Name} AS NewId;");
	}
	...
	Column? softDeleteCol, bool enableSoftDelete)
{
	...
	// IMPORTANT: Version param must be OPTIONAL so inserts don't require it.
	// Also: TIMESTAMP/ROWVERSION is mapped to varbinary(8) by SqlType().
	if (version != null)
		parms.Add($"@{version.Name} {SqlType(version)} = NULL");

	// Helpers
	string PkWhere(string alias) => string.Join(" AND ", pk.Select(p => $"{alias}.[{p.Name}] = @{p.Name}"));
	string PkColumnsCsv() => string.Join(", ", pk.Select(p => $"[{p.Name}]"));
	string InsertedPkColumnsCsv(string prefix) => string.Join(", ", pk.Select(p => $"{prefix}.[{p.Name}]"));
	...
	// Track affected PKs (works for single + composite)
	sb.AppendLine($"  {BuildPkTableVarDecl(pk)}".Replace("DECLARE @InsertedPk", "DECLARE @AffectedPk"));

	// UPDATE attempt (with concurrency if version provided)
	...
	// Only try UPDATE when we have something to set (or updated/version computed columns exist)
	if (updateSets.Any())
	{
		sb.AppendLine("  UPDATE t SET");
		sb.AppendLine("    " + string.Join(",\r\n    ", updateSets));
		sb.AppendLine($"  FROM {table} t");
		sb.AppendLine($"  WHERE {PkWhere("t")}");

		// Concurrency check only if caller supplied @Version (non-null)
		if (version != null)
		{
	...
	{
		sb.AppendLine("  -- No updatable columns detected; skip UPDATE attempt.");
		sb.AppendLine("  SELECT 0; -- placeholder");
		sb.AppendLine();
	}

	sb.AppendLine("  IF @@ROWCOUNT > 0");
	sb.AppendLine("  BEGIN");
	// For UPDATE path, record the PK(s) we updated so the final SELECT is based on @AffectedPk (composite-safe)
	sb.AppendLine($"    INSERT INTO @AffectedPk({PkColumnsCsv()})");
	sb.AppendLine($"    SELECT {PkColumnsCsv()};");
	sb.AppendLine("  END");
	sb.AppendLine("  ELSE");
	sb.AppendLine("  BEGIN");
	sb.AppendLine("    -- No rows updated. Decide: concurrency conflict vs insert");
	sb.AppendLine($"    IF EXISTS (SELECT 1 FROM {table} t WHERE {PkWhere("t")})");
	sb.AppendLine("    BEGIN");
	if (version != null)
	{
		sb.AppendLine($"      -- Row exists by PK but version mismatched (or other non-update reason)");
		sb.AppendLine("      THROW 50000, 'Record not found or version mismatch', 1;");
	}
	else
	{
		sb.AppendLine("      -- Row exists by PK; nothing updated (no sets) or other mismatch");
		sb.AppendLine("      THROW 50000, 'Record not found or update not possible', 1;");
	}
	sb.AppendLine("    END");
	sb.AppendLine();

	// INSERT path (capture PK via OUTPUT)
	...
	sb.AppendLine($"    OUTPUT {InsertedPkColumnsCsv("inserted")} INTO @AffectedPk({PkColumnsCsv()})");
	...
	// Return the affected row(s) using @AffectedPk join (composite-safe, insert-safe)
	sb.AppendLine($"  SELECT {BuildColumnList(columns, pkNames, softDeleteCol, enableSoftDelete)}");
	sb.AppendLine($"  FROM {table} t");
	sb.AppendLine($"  INNER JOIN @AffectedPk k ON " + string.Join(" AND ", pk.Select(p => $"t.[{p.Name}] = k.[{p.Name}]")) + ";");
...
// ─────────────────────────────────────────────
// Delete / Restore (composite PK safe)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Read procedures with explicit columns (composite PK safe)
// ─────────────────────────────────────────────
...
static string BuildPkTableVarDecl(List<Column> pk)
{
	// DECLARE @AffectedPk TABLE ([K1] type, [K2] type, ...)
	var cols = string.Join(", ", pk.Select(p => $"[{p.Name}] {SqlType(p)}"));
	return $"DECLARE @AffectedPk TABLE ({cols});";
}
