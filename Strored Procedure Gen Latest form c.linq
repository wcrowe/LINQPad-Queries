...
// Features:
// - Upsert (UPDATE+INSERT)  (MERGE-free by design)
...
// - Version param is OPTIONAL; concurrency enforced only if caller supplies it
// - Delete/Restore returns affected row(s) using OUTPUT (composite-PK safe)
//
// Notes:
// - TIMESTAMP/ROWVERSION cannot be used as a proc parameter type. This script maps it to VARBINARY(8).
// - MERGE intentionally avoided.
//
// ─────────────────────────────────────────────
void Main()
{
...
static (Column? Created, Column? Updated, Column? SoftDelete, Column? BuiltInVersion, Column? ManualVersion) DetectSpecialColumns(List<Column> columns)
{
	...
	var builtInVersion = columns.FirstOrDefault(c =>
...
// ─────────────────────────────────────────────
// Update (optional concurrency param)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Upsert (composite-PK safe, MERGE-free, correct concurrency)
// ─────────────────────────────────────────────
	...
	Column? softDeleteCol, bool enableSoftDelete)
{
	...
	// Return affected rows by PK join
...
// ─────────────────────────────────────────────
// Delete / Restore (OUTPUT-based returns; composite-PK safe)
// ─────────────────────────────────────────────
static string DeleteProc(string schema, string baseName, string table, List<Column> pk, Column? softDeleteCol, bool enableSoftDelete, List<Column> columns, List<string> pkNames)
{
	...
	else
	{
		...
		sb.AppendLine();
		sb.AppendLine("  SELECT @@ROWCOUNT AS RowsAffected;");
		sb.AppendLine($"  SELECT {BuildPkSelectList(pk)} FROM @AffectedPk; -- row is gone, return PKs only");
	}
...
{
	// You cannot declare proc params as TIMESTAMP/ROWVERSION.
	// Use VARBINARY(8) for both params and table variables.
	if (c.IsRowVersion ||
		c.TypeName.Equals("timestamp", StringComparison.OrdinalIgnoreCase) ||
		c.TypeName.Equals("rowversion", StringComparison.OrdinalIgnoreCase))
		return "varbinary(8)";
