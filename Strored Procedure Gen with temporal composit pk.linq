...
// ─────────────────────────────────────────────
// Upsert (FULL composite PK support + IDENTITY-in-composite PK output)
// ─────────────────────────────────────────────
	...
	Column? softDeleteCol, bool enableSoftDelete)
{
	...
	// FULL composite support: treat identity as output if it participates in the PK, even if PK is composite.
	var identityInPk = identity != null && pk.Any(p => p.Name.Equals(identity.Name, StringComparison.OrdinalIgnoreCase));
	...
	foreach (var p in pk)
	{
		if (identityInPk && identity != null && p.Name.Equals(identity.Name, StringComparison.OrdinalIgnoreCase))
	...
	// Composite PK WHERE (all PK columns), plus version if enabled
	...
	// If identity is part of PK (even composite), set it after INSERT so the final SELECT can use it.
	if (identityInPk && identity != null)
		sb.AppendLine($"  SET @{identity.Name} = SCOPE_IDENTITY();");
